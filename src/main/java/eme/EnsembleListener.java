...
public class EnsembleListener implements IAlgorithmListener, IConfigure 
{
	...
	protected void doIterationReport(IAlgorithm algorithm)
	{
		...
		{		
			...
			try {
				...
				File bestIndFile = new File(reportDirectory, bestIndFilename);
				File avgIndFile = new File(reportDirectory, avgIndFilename);
				...
				for(int i=0; i<inds.size(); i++) {
					buffer.append(Arrays.toString(((BinArrayIndividual)inds.get(i)).getGenotype()) + ": " + ((SimpleValueFitness)inds.get(i).getFitness()).getValue() + System.getProperty("line.separator"));
				...
				reportWriter.close();
				bestIndWriter.append(((SimpleValueFitness)inds.get(0).getFitness()).getValue() + System.getProperty("line.separator"));
				bestIndWriter.close();
				
				avgIndWriter.append(avgFitness + System.getProperty("line.separator"));
				avgIndWriter.close();
			} 
			catch (IOException e) {
    ...
    /**
     * Classify a multi-label dataset with a given ensemble classifier
     * 
     * @param mldata Multi-label dataset
     * @param classifier Ensemble classifier
     * @param file File to store the predictions
     */
    ...
    private void doReport(String reportFilename, String globalReportFilename, MultiLabelInstances dataset, EnsembleAlgorithm algorithm,  EnsembleClassifier classifier)
    {
    	...
    	try {
			...
			cab=cab+"number of evaluations, execution time, final coverage, final measure, final fitness, tableClassifiersSize, tableFitnessSize\n";
			file.write(System.getProperty("line.separator"));
			...
			bw.write(algorithm.finalEnsembleCoverage + "," + algorithm.finalEnsembleMeasure + "," + algorithm.finalEnsembleFitness + "," + algorithm.getTableClassifiersSize() + "," + algorithm.getTableFitnessSize());
    ...
    /**
     * Prepare the different evaluation measures
     * 
     * @param learner Multi-label learner
     * @param mlTestData Multi-label dataset
     * 
